...
A FiveM traffic-management script pack built for servers that want practical live traffic control without constantly editing configs or restarting the server.

This repository keeps the same split structure:

- **`full/traffic_control/`** → the **full version**, now updated to **v2.0**
- **`lite/traffic_control_lite/`** → the **lite version**, kept for servers that want a simpler setup
...
Current full version: **v2.0**

### Lite version (`lite/traffic_control_lite/`)
Use the lite version if you want a simpler setup focused on global traffic control.
...
## What v2.0 adds to the full version
...
- remove nearest prop
- clear my props
- prop sync cleanup reconciliation
- configurable per-player prop limit
- default prop limit: `20`
- cleaner server-side prop lifecycle for runtimes with limited native support
...
### Full version
- Included props are intentionally basic in v2.0
- Prop placement is designed for practical RP use, not full map-editor precision
